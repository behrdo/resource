...
#plot
ggplot(RL_tot, aes(x = as.Date(JDay, origin = as.Date("2013-01-01")), y = tot_m2, 
...
#for max values comparing blk and BP for all years and all depth------------------------------
...
RLD3 <- RLD2 %>%group_by(pore_kind,crop, Year, treatment, rainout.shelter, depth) %>% filter(JDay == max(JDay))
#-> this data frame is used to calculate the p values! not the one below!!
RLD2 <- RLD3 %>% group_by(pore_kind, crop, Year, treatment, rainout.shelter, depth) %>% 
...
ggplot(RLDP, aes(x = depth, y = percentage, colour = interaction(rainout.shelter, treatment),
  ...
  labs(x = "Soildepth [cm]", y = "Share of Roots in Biopores [%]") +
  scale_x_reverse()+
  coord_flip()+
...
ggplot(RL_t_s, aes(x = as.Date(JDay, origin = as.Date("2013-01-01")), y = tot_m2, colour = interaction(rainout.shelter, treatment),
                   group = interaction(treatment, rainout.shelter))) + 
  geom_point() + geom_line() +
