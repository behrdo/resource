...
-------------
#for max values comparing blk and BP for all years and all depth-----not nice jet+Fehler--------------------------
#FEHLER!!!
...
ggplot(RLD2, aes(x = depth, y = cm3, colour = interaction(rainout.shelter, pore_kind),
  ...
  labs(x = bquote("Soil Depth [cm]"), y= "Rootlength [m *" ~cm^-3 ~"]" , title = "Total Rootlegnth") +
  theme_bw() +
  scale_x_reverse()+
  coord_flip()+
  theme(axis.text = element_text(size = 10), 
